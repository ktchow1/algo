...
//  std::cout <<  alg::is_inherit_v<base,    derpriv>; // cannot compile
    ...
    std::string str;
    assert((alg::sfinae_interface(str) == true));
    assert((alg::sfinae_interface(std::move(str)) == false));
    assert((alg::sfinae_interface(std::string{ }) == false));
    ...
    static_assert(alg::is_base_of_v<base,    derpriv> == false, "failed to use is_base_of"); // can compile, but result incorrect
