...
{
public:
    base(bool debug, int id) : _debug(debug), _id(id) { if (_debug) std::cout << "\nbase constructor"; }
...
{
public:
    derived0(bool debug, int id) : base(debug,id) { if (_debug) std::cout << "\nderived0 constructor";  }
...
{
public:
    derived1(bool debug, int id) : base(debug,id) { if (_debug) std::cout << "\nderived1 constructor";  }
...
public:
    void operator()(base* ptr) const
    {
        ...
        std::cout << "\ncustom_deleter operator for id " << ptr->get_id();
        delete ptr;
    }
};
