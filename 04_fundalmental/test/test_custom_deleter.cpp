...
namespace alg
{
    ...
    class stack_pool
    {
        ...
        {
        public:
            custom_deleter(stack_pool<T,N>& pool, std::uint32_t index) : m_pool(pool), m_index(index)
            {
            }
            ...
            {
                std::stringstream ss;
                ss << "custom delete from stack_pool index " << m_index; 
        ...
        auto make_unique(ARGS&&... args)
        {
            for(std::uint32_t n=0; n!=N; ++n)
            {
                if (m_flags[n])    
                {
                    ...
                    std::unique_ptr<T, custom_deleter> ptr(nullptr, custom_deleter{*this, n});
                    ptr.reset
                    (
            ...
            return std::unique_ptr<T, custom_deleter>{nullptr, custom_deleter{*this, N}};
        }
...
// ********************************************************* //
// *** Test custom deleter for unique_ptr and stack_pool *** //
// ********************************************************* //
...
void test_custom_deleter_for_stack_pool()
{
    struct A
    {
        std::uint32_t m_x;
    ...
    std::uint32_t n = 0;
    assert(alg::deleter_invoked[n] == "custom delete from stack_pool index 3");     ++n;
    assert(alg::deleter_invoked[n] == "custom delete from stack_pool index 2");     ++n;
    assert(alg::deleter_invoked[n] == "custom delete from stack_pool index 4");     ++n;
    assert(alg::deleter_invoked[n] == "custom delete from stack_pool index 3");     ++n;
    assert(alg::deleter_invoked[n] == "custom delete from stack_pool index 2");     ++n;
    assert(alg::deleter_invoked[n] == "custom delete from stack_pool index 1");     ++n;
    assert(alg::deleter_invoked[n] == "custom delete from stack_pool index 0");     ++n;
    print_summary("custom deleter for stack_pool", "succeeded");
}
