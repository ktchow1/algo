...
// ******************************************* //
// Do NOT floor more than std::chrono::days
// flooring std::chrono::months or
// flooring std::chrono::years 
// will result in unexpected day & time shift.
// ******************************************* //
                         ...
                         const alg::time_pod& pod) // expected answer
{
    ...
    std::cout << "\nconvert and compare : ts="  << alg::to_string(ts0) 
                                    << ", tp="  << alg::to_string(tp0, true) 
                                    << ", pod=" << alg::to_string(pod0) << ", ";
    if (pod == pod0 && tp0 == tp1)
    {
    ...
    else 
    {
        ...
        std::cout << "\ntp0 = " << alg::to_string(tp0, true) << std::flush;
        std::cout << "\ntp1 = " << alg::to_string(tp1, true) << std::flush;
    }

    return pod == pod0 && tp0 == tp1;
}
...
void test_conversion()
{
    ...
    for(int n=1; n!=31; ++n, tp+=std::chrono::days(1)) assert(convert_and_compare(tp, alg::time_pod { 2000,1,n,12,30,0,1234567 }));
    assert(convert_and_compare(tp, alg::time_pod { 2000,1,31,12,30,0,1234567 }));
    tp+=std::chrono::days(1);

    for(int n=1; n!=29; ++n, tp+=std::chrono::days(1)) assert(convert_and_compare(tp, alg::time_pod { 2000,2,n,12,30,0,1234567 }));
    assert(convert_and_compare(tp, alg::time_pod { 2000,2,29,12,30,0,1234567 }));
    tp+=std::chrono::days(1);

    for(int n=1; n!=31; ++n, tp+=std::chrono::days(1)) assert(convert_and_compare(tp, alg::time_pod { 2000,3,n,12,30,0,1234567 }));
    assert(convert_and_compare(tp, alg::time_pod { 2000,3,31,12,30,0,1234567 }));
    tp+=std::chrono::days(1);

    for(int n=1; n!=30; ++n, tp+=std::chrono::days(1)) assert(convert_and_compare(tp, alg::time_pod { 2000,4,n,12,30,0,1234567 }));
